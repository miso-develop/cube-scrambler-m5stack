...
1. `PROJECT.md`、対象`[Task]`、親`[Spec]`、参照Decision / artifact / comments、関連コードとtestを確認する。
...
4. 選択Taskと既存挙動維持に必要な最小変更だけを行う。設計判断には`codebase-design`、over-engineering判断には`ponytail`、test-firstが適切なら`tdd`を使う。
